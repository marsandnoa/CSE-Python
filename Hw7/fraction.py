...
class Fraction:
    ...
    def __init__(self, num: int, d="stringinput"):
        ...
        if(d!="stringinput"):
          ...
          holder=Fraction.from_str(0,num)
          self.num=holder.num
          self.d=holder.d
    ...
    def from_str(cls, str_rep: str):
      ...
      try:
        for i in str_rep:
          if(i=="/"):
            return Fraction(int(str_rep[0:index]),int(str_rep[index+1:len(str_rep)]))
          if(i=="."):
            decimals=len(str_rep)-index
            decimals=10**(decimals-1)
            return Fraction(int(str_rep[0:index])*(10**decimals)+int(str_rep[index+1:len(str_rep)]),decimals)
