...
class Graph(ABC):
    ...
    def depth_first_search(self, start=None):
      ...
      if(len(self.adjacent_to(start))==0):
        pass
      else:
        fringeset=self.adjacent_to(start)
        for i in fringeset:
          fringelist.append(i)
        ...
        while(len(fringelist)!=0):
          node=fringelist.pop()
          yield node
          localset=self.adjacent_to(node)
          if len(localset)!=0:
            for i in localset:
              if(len(fringeset.intersection({i}))==0):
                #this should pass if the node being added is not already in the fringe
                fringeset.add(i)
                fringelist.append((i))
